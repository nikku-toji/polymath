...
/* ----------------------------------------------------------------
   ...
   reinforces with spaced repetition. Powered by live Claude calls.
-----------------------------------------------------------------*/
...
export default function App() {
  ...
  const [mastered, setMastered] = useState([]); // concept ids
  ...
  const [history, setHistory] = useState([]); // {q, a, verdict, feedback}
  const [conceptDone, setConceptDone] = useState(false);
  const [correctCount, setCorrectCount] = useState(0);

  // review
  const [cards, setCards] = useState([]); // {front, back, box, concept}
  ...
  async function buildPath() {
    ...
    try {
      ...
      setError("Couldn't build the path. Try again or rephrase the skill.");
    } finally {
      setBusy(false);
    }
  }

  /* ---------- open a concept (teach) ---------- */
                ...
                {!conceptDone && (
                  <div className="pm-qa">
                    ...
                    <div className="pm-qa-foot">
                      <span className="pm-hint"><CornerDownLeft size={12} /> ⌘/Ctrl + Enter</span>
